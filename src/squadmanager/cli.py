...
def cli():
  ...
  restart        Redémarrer l'interface locale de CrewAI Studio (Streamlit)'''
    )
    ...
    stop = studio_sp.add_parser("stop", help="Arrêter l'interface locale de CrewAI Studio (Streamlit)")
    restart = studio_sp.add_parser("restart", help="Redémarrer l'interface locale de CrewAI Studio (Streamlit)")
    ...
    # crew commands
        ...
        if args.studio_cmd == "stop":
            ...
            sys.exit(0)
        if args.studio_cmd == "restart":
            # Redémarrage de l'UI locale via Streamlit
            proc = subprocess.run(['netstat', '-ano'], capture_output=True, text=True, check=True)
            for line in proc.stdout.splitlines():
                parts = line.split()
                if len(parts) >= 5 and parts[1].endswith(':8501') and parts[3] == 'LISTENING':
